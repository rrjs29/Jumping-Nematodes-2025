...
class wind_fit_joint:
    def __init__(self, t, X, x0, init_guess, scales, m_flag = False,
                 RW_sigma = 0.1, para_var = 1e1, burn_in = 500):      
    ...
    def ode(self, x, t, c1, c4, c5):
        # inferred parameters
        a_w = c1*self.scales[0] + self.IG[0] # [m], hydrodynamical radius of the worm, roughly 100~um
        U_inf = c4*self.scales[3] + self.IG[3]
        Lx_0 = c5*self.scales[4] + self.IG[4]
        
        # fixed parameters
        pi = np.pi # [-]
        g0 = 9.81; # [m/s^2]
        eta = 1.849e-5; # [Pa.s], dynamic viscosity of air
        nu = 1.562e-5; # [m^2/s], kinematic viscosity of air
        rho_w = 1e3; # [kg/m^3], density of the worm
        
        # derived parameters from fixed parameters
        m1 = rho_w*pi*(12e-6)**2.0*500e-6; # [kg], mass of the worm, pi*r^2*L, overnight worm
        m2 = rho_w*pi*(15e-6)**2.0*400e-6; # [kg], mass of the worm, pi*r^2*L, fresh worm
        
        if self.m_flag:
            m_w = m2 # overnight worms are longer and thinner
        else:
            m_w = m1 # freshly hatched worms are shorter and thicker
            
        # variables that changes over time
        x1, y1, u1, v1 = x
        
        # Blasius solution
        L_x = np.abs(Lx_0) + abs(x1)
                    
        zeta = y1*np.sqrt(U_inf/nu/L_x)
                
        # get U, V from Blasius solution
        U = U_inf*self.f1(zeta)
        V = 0.5*np.sqrt(nu*U_inf/L_x)*(self.f1(zeta)*zeta-self.f0(zeta))       
        
        dxdt = [u1, v1,
                -6*pi*eta*a_w*(u1-U)/m_w,
                -6*pi*eta*a_w*(v1-V)/m_w - g0]
        
        return dxdt
    
    def f0(self, zeta):
        f0_val = zeta - 1.8760 + 1.4564*np.exp(-zeta) + 1.2956*zeta*np.exp(-zeta) + 0.4196*np.exp(-2*zeta);
        return f0_val
    
    def f1(self, zeta):
        f1_val = 1 - 1.4564*np.exp(-zeta) + 1.2956*(np.exp(-zeta)*(1-zeta)) - 2*0.4196*np.exp(-2*zeta);
        return f1_val
    ...
    def log_likelihood(self, theta, log_sigma):
        ...
        mu = odeint(self.ode, init_cond, self.t, args = tuple([c1,c4,c5]))
...
#         samples = np.asarray(self.sampler.samples)[self.burn_in:,:]
        ...
        ylabels = [r"$a_h\ [\mu$m]", r"$u_x$ [m/s]", r"$u_y$ [m/s]", r"$U_{inf}$ [m/s]", r"$Lx_0$ [mm]"]
        plot_scales = [1e6, 1., 1., 1., 1e3]
        length = len(ylabels)
        
        for kk in range(length):
            plt.subplot(length+1,1,kk+1)
            plt.plot(plot_scales[kk]*(self.scales[kk]*samples[:,0,kk]+self.IG[kk]),color= 'tab:blue')
            plt.axhline(plot_scales[kk]*(self.scales[kk]*np.mean(samples[self.burn_in:,0,kk],0)
                                         +self.IG[kk]),color='k', linestyle='--')
            plt.ylabel(ylabels[kk], fontsize=16)
            plt.tick_params(which = 'major',direction = 'in',length=8, width=1, labelsize=16)
            plt.grid(True)
            plt.xticks(xticks,[])
        ...
        plt.subplot(length+1,1,length+1)
    ...
    def show_prior(self, show = True):
        ...
        u_init = self.IG[1] # [m/s], initial x velocity, initial guess is measured
        v_init = self.IG[2] # [m/s], initial y velocity, initial guess is measured
        
        init_cond = [self.x0[0], self.x0[1], u_init, v_init]
        
        # Integrate to get results
        IG_prior = odeint(self.ode, init_cond, self.t, args = tuple([0,0,0]))
    ...
    def show_mean_sol(self, show = True, long_t = 0):
        ...
        # read theta to initial condition
        c1, c2, c3, c4, c5 = mean_samples.flatten()
        u_init = c2*self.scales[1] + self.IG[1] # [m/s], initial x velocity, initial guess is measured
        v_init = c3*self.scales[2] + self.IG[2] # [m/s], initial y velocity, initial guess is measured
        init_cond = [self.x0[0], self.x0[1], u_init, v_init]
        
        # exam loss in stress sigma
        mean_sol = odeint(self.ode, init_cond, self.t, args = tuple([c1, c4, c5]))
        self.mean_sol = mean_sol
        
        # generate longer time for integration  
        dt = 1e-4
        t_longer = np.arange(len(self.t) + long_t) * dt
        long_sol = odeint(self.ode, init_cond, t_longer, args = tuple([c1, c4, c5]))
    ...
    def show_coeff(self, show_PDF = True, view = [30,-60]):
        ...
        U_inf_samples = normalized_samples[:,0,3]*1.
        Lx0_samples = np.abs(normalized_samples[:,0,4])*1e3

        nu = 1.562e-5; # [m^2/s], kinematic viscosity of air
        delta_samples = 5.0*np.sqrt((Lx0_samples*1e-3+10e-3)*nu/U_inf_samples)*1e3
        ...
        coeff = np.vstack((a_w_samples,u_x_samples,u_y_samples,U_inf_samples,Lx0_samples,sigma_samples))
        ...
        print('U_inf = %1.4f +/- %1.4f m/s' %(np.mean(U_inf_samples),np.std(U_inf_samples)))
        
        print('Lx_0 = %1.4f +/- %1.4f mm' %(np.mean(Lx0_samples),np.std(Lx0_samples)))
        print('delta = %1.4f +/- %1.4f mm' %(np.mean(delta_samples),np.std(delta_samples)))
